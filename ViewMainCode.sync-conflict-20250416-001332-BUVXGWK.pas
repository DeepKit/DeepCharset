...
begin
  ...
  if SameText(DetectedEncoding, 'UTF-8') then
    ShowFileContent(SelectedFile, TEncoding.UTF8, DetectedEncoding, HasBOM)
  else if SameText(DetectedEncoding, 'GB2312') or SameText(DetectedEncoding, 'GBK') then
    ShowFileContent(SelectedFile, TEncoding.GetEncoding(936), DetectedEncoding, HasBOM)
  else if SameText(DetectedEncoding, 'ASCII') then
    ShowFileContent(SelectedFile, TEncoding.ASCII, DetectedEncoding, HasBOM)
  else
    ShowFileContent(SelectedFile, TEncoding.Default, DetectedEncoding, HasBOM);
end;
...
begin
  try
    ...
    // 加载文件内容
    SynEditForm.LoadFile(FileName, Encoding);
    SynEditForm.SetFileInfo(FileName, DetectedEncoding, HasBOM);
    
    // 显示窗体
    SynEditForm.Show;
    
    // 记录日志
    Log('成功打开文件: ' + FileName + ' (编码: ' + DetectedEncoding + ', BOM: ' + BoolToStr(HasBOM, True) + ')');
  except
    on E: Exception do
    begin
