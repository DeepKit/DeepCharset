...
uses
  UtilsEncodingBOM_Improved, UtilsEncodingUTF8Detector_Improved, UtilsEncodingConfig;
...
begin
  ...
  // 纯 ASCII 缓冲在多编码场景下高度歧义，不应被判为高置信度 UTF-8 / 中文编码
  HasNonAscii := False;
  for var B in Buffer do
  begin
    if B >= $80 then
    begin
      HasNonAscii := True;
  ...
  begin
    ...
    // 纯 ASCII（无高位字节）时，将 UTF-8 置信度钳制在较低值，避免在多编码场景下误报为高置信度
    if (not HasNonAscii) and (Result.Confidence > 0.5) then
      Result.Confidence := 0.5;

    Result.UTF8Confidence := Result.Confidence;
  ...
  // 使用配置的阈值而不是硬编码的 MIN_CONFIDENCE
  var ConfiguredThreshold := TEncodingDetectionConfig.MinChineseConfidence;

  if MaxScore < ConfiguredThreshold then
  begin
  ...
  begin
    ...
    // 根据地区特征和置信度确定编码
    if HasTaiwanFeature and (FinalBig5Score >= ConfiguredThreshold) then
    begin
      Result.Encoding := ENCODING_BIG5;
      Result.Confidence := Max(FinalBig5Score, 0.85);
    end
    else if HasMainlandFeature and (FinalGB18030Score >= ConfiguredThreshold) then
    begin
      Result.Encoding := ENCODING_GB18030;
      Result.Confidence := Max(FinalGB18030Score, 0.85);
    end
    else if HasMainlandFeature and (FinalGBKScore >= ConfiguredThreshold) then
    begin
...
begin
  ...
  try
    ...
    // 限制读取大小，避免处理过大的文件
    ReadSize := Min(FileStream.Size, FILE_READ_LIMIT);
...
begin
  ...
  try
    ...
    // 读取流内容进行分析
    ReadSize := Min(Stream.Size, MAX_SAMPLE_SIZE);
    SetLength(Buffer, ReadSize);
