...
begin
  ...
  try
    ...
    try
      ...
      // 获取文件列表 - 使用FIncludeSubdirs和FMaxDepth参数
      ...
      begin
        ...
        ThresholdCaptions := ['2,000', '5,000', '2万', '10万', '50万'];
        ...
        if ti >= 0 then
        begin
          var Msg := Format('已扫描到 %d 个文件（超过 %s 阈值），是否继续？',
            [FileCount, ThresholdCaptions[ti]]);
          if Application.MessageBox(PChar(Msg), '文件数量确认',
...
begin
  ...
  // 获取系统语言（由 LanguageManager.Initialize 通过 Windows API 检测）
  SystemLangCode := ControllerLanguage.GetCurrentLanguage;
  Log('Windows 系统语言: ' + SystemLangCode);

  // 根据系统语言自动选中匹配项
  MatchedLangCode := '';
  ComboBox1.ItemIndex := 0; // 默认选中第一项（English）
  ...
  begin
    ...
    // 精确匹配失败时，尝试语言前缀匹配（如 zh-CN → zh-TW）
    ...
    begin
      ...
      Log('自动跟随 Windows 语言: ' + MatchedLangCode);
    end
    else
    begin
...
begin
  ...
  if FIncludeSubdirs then
  begin
    Log('已启用子目录搜索 - 深度: ' + IntToStr(FMaxDepth));
...
begin
  ...
  Log('扫描深度已调整为: ' + IntToStr(FMaxDepth));
  if FIncludeSubdirs then
    UpdateFileGrid(FSelectedFolder);
end;
