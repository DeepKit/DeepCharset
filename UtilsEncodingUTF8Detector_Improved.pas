...
uses
  UtilsEncodingBOM_Improved, UtilsEncodingConfig;
...
begin
  ...
  // 根据置信度调整最终结果（使用配置阈值）
  if Result.Confidence >= TEncodingDetectionConfig.MinUTF8Confidence then
    Result.IsUTF8 := True
  else
    Result.IsUTF8 := False;
end;
...
begin
  ...
  // 使用配置的 UTF-8 最小置信度阈值，而不是写死 0.95
  var Confidence: Double := 0.0;
  if (ValidCount + InvalidCount) > 0 then
    Confidence := ValidCount / (ValidCount + InvalidCount);

  Result := (ValidCount > 0) and (Confidence >= TEncodingDetectionConfig.MinUTF8Confidence);

  // 如果有连续的长UTF-8序列，也认为是UTF-8（保持原有保护逻辑）
  if (not Result) and (MaxConsecutiveValid >= 15) then
    Result := True;
