...
type
  ...
  private
    const
      MIN_UTF8_CONFIDENCE = 0.75; // 最小UTF-8置信度
...
begin
  ...
  if AsianLanguageConfidence > 0.5 then
    // 如果有明显的亚洲语言特征，提高权重
    FinalConfidence := 0.6 * BasicConfidence + 0.2 * DistributionConfidence + 0.1 * FrequencyConfidence + 0.1 * AsianLanguageConfidence
  else
...
begin
  ...
  if (not Result) and (MaxConsecutiveValid >= 10) then
    Result := True;
end;
