...
uses
  ...
  System.TypInfo, Vcl.Clipbrd, ImprovedEncodingDetector, ImprovedEncodingConverter;
...
Type
  ...
  private
    ...
    // 检测文件编码
    function DetectFileEncoding(const FileName: string; out EncodingName: string; out HasBOM: Boolean): Boolean;

    // 检查文件是否有BOM
    function HasBOM(const FileName: string; Encoding: TEncoding = nil): Boolean;

    // 转换文件编码
    function ConvertFileEncoding(const SourceFile: string; TargetEncoding: TEncoding; AddBOM: Boolean = False): Boolean;

    // 编码检测和转换组件
    FEncodingDetector: TEncodingDetector;
    FEncodingConverter: TEncodingConverter;
...
begin
  ...
  // 设置日志回调
  FEncodingConverter := TEncodingConverter.Create;
...
begin
  ...
  begin
    var NodeData := Integer(TreeViewEncodings.Selected.Data);
    var WithBOM: Boolean;
    Encoding := FEncodingDetector.GetEncodingByIndex(NodeData, WithBOM);
  end;
  ...
  try
    ...
    begin
      // 使用新的转换方法
      if ConvertFileEncoding(AllFiles[i], Encoding, True) then
        Inc(SuccessCount);
    end;
...
// 转换文件编码
function TForm1.ConvertFileEncoding(const SourceFile: string; TargetEncoding: TEncoding; AddBOM: Boolean = False): Boolean;
var
...
begin
  ...
  try
    // 检测源文件编码
    if not DetectFileEncoding(SourceFile, SourceEncodingName, HasBOM) then
    begin
      Log(Format('无法检测源文件编码: %s', [SourceFile]));
      Exit;
    end;

    // 使用新的编码转换器转换文件
    ConversionResult := FEncodingConverter.ConvertFileEncoding(SourceFile, TargetEncoding, AddBOM);
    ...
    // 记录日志
    if Result then
      Log(Format('成功转换文件: %s, 从 %s 到 %s, BOM: %s',
        [SourceFile, SourceEncodingName, FEncodingDetector.GetEncodingName(TargetEncoding), BoolToStr(AddBOM, True)]))
    else
      Log(Format('转换文件失败: %s, 错误: %d', [SourceFile, ConversionResult.ErrorCount]));
  except
    on E: Exception do
    begin
      Log(Format('转换文件出错: %s - %s', [SourceFile, E.Message]));
      Result := False;
    end;
  end;
end;
...
begin
  ...
  Encoding := FImprovedEncodingDetector.GetSelectedEncoding;
  ...
  try
    ...
    begin
      ...
      // 使用新的转换方法
      if ConvertFileEncoding(FilePath, Encoding, True) then
      begin
        Inc(SuccessCount);
...
begin
  ...
  // 检测文件编码
  if not DetectFileEncoding(FilePath, EncodingName, HasBOM) then
    EncodingName := '未知';
...
// 检测文件编码
function TForm1.DetectFileEncoding(const FileName: string; out EncodingName: string; out HasBOM: Boolean): Boolean;
var
...
begin
  ...
  try
    ...
    // 获取编码名称和BOM信息
    EncodingName := DetectionResult.Name;
    HasBOM := DetectionResult.HasBOM;

    // 记录日志
    Log(Format('检测文件编码: %s, 结果: %s, BOM: %s, 置信度: %.2f',
      [FileName, EncodingName, BoolToStr(HasBOM, True), DetectionResult.Confidence]));

    Result := True;
  except
    on E: Exception do
    begin
      Log(Format('检测文件编码出错: %s - %s', [FileName, E.Message]));
      EncodingName := '未知';
      HasBOM := False;
      Result := False;
    end;
...
begin
  ...
  try
    ...
    // 检查是否有BOM
    Result := DetectionResult.HasBOM;

    // 记录日志
    Log(Format('检测文件BOM: %s, 结果: %s', [FileName, BoolToStr(Result, True)]));
  except
    on E: Exception do
    begin
      Log(Format('检测文件BOM出错: %s - %s', [FileName, E.Message]));
      Result := False;
    end;
  end;
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        // 检查是否为UTF-8 BOM节点
        if (NodeData >= 0) and (NodeData < Length(ModelEncoding.ENCODINGS)) then
        begin
          // 检查编码是否为UTF-8且有BOM
          if (ModelEncoding.ENCODINGS[NodeData].CodePage = 65001) and
             (ModelEncoding.ENCODINGS[NodeData].HasBOM) then
          begin
            // 选中该节点
    ...
    begin
      ...
      begin
        ...
        begin
          // 查找普通UTF-8
          if (ModelEncoding.ENCODINGS[NodeData].CodePage = 65001) and
             (not ModelEncoding.ENCODINGS[NodeData].HasBOM) then
          begin
            TreeViewEncodings.Selected := Node;
            Node.MakeVisible;
...
begin
  ...
  FUIHelper.SetupEncodingList(TreeViewEncodings, FEncodingDetector);
...
begin
  ...
  try
    ...
    // 清空并重新构建编码列表
    FUIHelper.SetupEncodingList(TreeViewEncodings, FImprovedEncodingDetector);
...
begin
  ...
  // 获取选中的编码信息
  if not FImprovedEncodingDetector.GetSelectedTargetEncoding(TreeViewEncodings.Selected, TargetEncoding) then
  begin
  ...
  // 开始日志缓冲
  Log('开始批量转换 ' + IntToStr(Length(SelectedFiles)) + ' 个文件到 ' + TargetEncoding.Name + '...');
  StartLogBuffering;

  // 执行转换
  Screen.Cursor := crHourGlass;
  try
    SuccessCount := FImprovedEncodingConverter.ConvertFiles(
      SelectedFiles,
      TargetEncoding,
    ...
    if SuccessCount < Length(SelectedFiles) then
      Log(Format('注意: %d 个文件未能成功转换', [Length(SelectedFiles) - SuccessCount]));
...
var
  FileName: string;
  TargetEncoding: TTargetEncoding;
  DetectedInfo: TDetectedEncodingInfo;
begin
  // 获取选中的文件
  ...
  FileName := TPath.Combine(FSelectedFolder, StringGrid1.Cells[0, FSelectedRow]);
  if not FileExists(FileName) then
  begin
    ShowLocalizedMessage('MsgFileNotFound');
    Exit;
  end;
  ...
  // 检测当前编码
  if not FImprovedEncodingDetector.DetectFileEncoding(FileName, DetectedInfo) then
  begin
    ShowLocalizedMessage('MsgDetectFailed');
    Exit;
  end;
  ...
  Screen.Cursor := crHourGlass;
  try
    if FImprovedEncodingConverter.ConvertFile(FileName, TargetEncoding) then
    begin
      Log(Format('文件转换成功: %s', [FileName]));
      Log(Format('- 从: %s', [DetectedInfo.EncodingName]));
      Log(Format('- 到: %s', [TargetEncoding.Name]));
      UpdateSingleFileInGrid(FileName);
