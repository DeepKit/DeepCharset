...
type
  ...
  private
    ...
    function OldDetectFileEncoding(const FileName: string; out HasBOM: Boolean): string;
  public
...
begin
  ...
  try
    // 使用iconv库进行转换
    if TIconv.LibraryLoaded then
    begin
      // 获取目标编码名称
      CodePage := TargetEncoding.CodePage;
      EncodingName := TEncodingUtils.GetEncodingName(CodePage);
      
      // 执行转换
      Result := TEncodingUtils.ConvertFileEncoding(
        SourceFile, TargetFile, EncodingName, AddBOM);
      ...
      begin
        ...
        else if (CodePage = 936) then
          FLogCallback('成功转换到GB2312: ' + SourceFile)
        else
          FLogCallback(Format('成功转换到%s编码: %s', [EncodingName, SourceFile]));
      end;
    end
    else
    begin
      ...
      if TargetEncoding.CodePage = 936 then
        Result := ConvertFileToGB2312(SourceFile, TargetFile)
      else
        Result := ConvertFileToUTF8(SourceFile, TargetFile);
...
begin
  ...
  if TIconv.LibraryLoaded then
  begin
    EncodingInfo := TEncodingUtils.DetectFileEncoding(FileName);
    HasBOM := EncodingInfo.HasBOM;
    Result := EncodingInfo.EncodingName;
    
    // 提供更友好的中文名称
    if SameText(Result, 'UTF-8') then
    begin
      if HasBOM then
        Result := 'UTF-8 BOM'
      else
    ...
    else if SameText(Result, 'GB2312') or SameText(Result, 'GBK') then
      Result := '简体中文 (GB2312)'
    else if SameText(Result, 'BIG5') then
      Result := '繁体中文 (BIG5)'
    else if SameText(Result, 'ASCII') then
      Result := 'ASCII'
    else if SameText(Result, 'ANSI') then
      Result := 'ANSI/未知';
    
    // 处理特殊文件类型
    if ExtractFileExt(FileName).ToLower() = '.bin' then
      Result := '二进制'
    else if ContainsText('.exe;.dll;.obj;.so;.lib;', ExtractFileExt(FileName).ToLower() + ';') then
      Result := '二进制';
  end
  else
  begin
    // 回退到原来的检测方法
    Result := OldDetectFileEncoding(FileName, HasBOM);
  end;
end;

function TFileHelper.OldDetectFileEncoding(const FileName: string; out HasBOM: Boolean): string;
var
  Encoding: TEncoding;
  Stream: TFileStream;
  Buffer: TBytes;
