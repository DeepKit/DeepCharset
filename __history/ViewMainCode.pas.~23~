...
uses
  ...
  ControllerEncoding, HelperLanguage, ModelLanguage, Winapi.ShlObj, ViewMemo, Vcl.Themes, ViewSynEdit,
...
begin
  ...
  // 注意：DirectoryListBox1没有Caption属性
  // 如果需要显示目录标签，请添加一个标签控件
  ...
  begin
    // 重新加载编码列表
    FEncodingModel.ReloadEncodings;

    // 刷新编码列表显示
    if Assigned(FUIHelper) and Assigned(TreeViewEncodings) then
...
begin
  ...
  // 记录用户选择的语言
  Log('用户选择语言: ' + FLanguageComboBox.Items[Index] + ' (' + LangCode + ')');
...
begin
  ...
  try
    ...
    // 添加支持的语言
    LangList.Add('简体中文=zh-CN');
    LangList.Add('English=en');
    LangList.Add('繁體中文=zh-TW');
    LangList.Add('Français=fr');

    Log('发现' + IntToStr(LangList.Count) + '种语言');

    // 填充下拉框
    for i := 0 to LangList.Count - 1 do
    begin
      // 解析格式：名称=代码
      var Parts := LangList[i].Split(['=']);
      ...
      begin
        ...
        // 添加语言到下拉框
        FLanguageComboBox.Items.Add(LangName);

        // 如果是当前语言，设置为选中
        if LangCode = LanguageManager.CurrentLanguage then
        begin
          FLanguageComboBox.ItemIndex := i;
          Log('当前语言: ' + LangName + ' (' + LangCode + ')');
        end;
      end;
    end;
  finally
...
begin
  ...
  if LangCode = 'zh-CN' then
    Index := 0
  else if LangCode = 'en' then
    Index := 1
  else if LangCode = 'zh-TW' then
    Index := 2
  else if LangCode = 'fr' then
    Index := 3;

  if (Index >= 0) and (Index < FLanguageComboBox.Items.Count) then
  begin
    if FLanguageComboBox.ItemIndex <> Index then
    begin
      Log('更新语言下拉框选中项: ' + FLanguageComboBox.Items[Index]);
      FLanguageComboBox.ItemIndex := Index;
    end;
  end;
...
begin
  ...
  // 记录获取到的语言字符串
  Log('获取到的语言字符串:');
  Log('WindowTitle: ' + LangStrings.WindowTitle);
  Log('BtnConvert: ' + LangStrings.BtnConvert);
  Log('BtnSingleFile: ' + LangStrings.BtnSingleFile);
  Log('BtnRefresh: ' + LangStrings.BtnRefresh);
  Log('BtnClose: ' + LangStrings.BtnClose);
  Log('BtnToggleSelect: ' + LangStrings.BtnToggleSelect);
  if Assigned(btnSVG2ICON) then
    Log('BtnSVG2ICON: ' + LangStrings.BtnSVG2ICON);
  ...
  Log('标签文本状态: 语言=' + Label1.Caption);
  Log('表格标题状态: 文件名=' + StringGrid1.Cells[1, 0]);
end;
...
begin
  // 直接调用SwitchToLanguageCode方法切换到中文
      ...
      begin
        ...
        begin
          ...
          if (FEncodingModel.Encodings[NodeData].CodePage = 65001) and
             (FEncodingModel.Encodings[NodeData].HasBOM) and
             (Pos('UTF-8 BOM', FEncodingModel.Encodings[NodeData].Name) > 0) then
          begin
            // 选中该节点
