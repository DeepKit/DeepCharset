...
- 背景与目�?
  ...
  - CLI/UI 均应输出简明的人类可读信息；详细信息写入日�?报告�?

- 上下文结构（UtilsExceptionContext�?
  - TExceptionContext.Operation：发生操作（�?ProcessSingleFile/Detect/Convert 等）
  - TExceptionContext.FilePath：当前处理的文件路径
  - TExceptionContext.Details：额外说明（目标编码、阶段信息等�?
  - ToMessage：将 Original Message 包裹�?“原始信�?| op=..., file=..., details=...�?

- CLI 集成（ControllerCommandLine�?
  - ProcessSingleFile 捕获到异常后，使�?TExceptionContext 包裹消息并输出到控制台与报告�?
  - --report 输出 JSON/XML 中的 error 字段使用带上下文的消息�?

- UI 日志
  - 取消动作、扫�?检�?转换异常均记录明确信息�?
...
- 已覆盖模块（当前进度�?
  ...
  - UI 层（ViewMainCode）通过日志呈现，不直接抛异�?
