...
type
  ...
  private
    ...
    /// 检查字节数组是否是UTF-8编码
    /// </summary>
    class function IsUTF8Buffer(const Buffer: TBytes; out HasBOM: Boolean): Boolean;
...
begin
  ...
  try
    ...
    try
      ...
      // 读取文件内容
      SourceStream.Position := BOMResult.BOMSize; // 跳过BOM（如果有）
      SetLength(Buffer, SourceStream.Size - BOMResult.BOMSize);
      
      if Length(Buffer) > 0 then
        SourceStream.ReadBuffer(Buffer[0], Length(Buffer));
        
      // 添加BOM
      var BufferWithBOM := AddUTF8BOM(Buffer);
...
begin
  // 使用BOM检测器的AddBOM方法
  Result := TEncodingBOMDetector_Improved.AddBOM(Buffer, bomUTF8);
end;
...
begin
  ...
  try
    ...
    Result.HasBOM := (BOMResult.BOMType = bomUTF8);
    
    // 如果已经是UTF-8+BOM，并且源文件和目标文件相同，则不需要处理
    if (SourceEncoding = ENCODING_UTF8_BOM) and (SourceFileName = TargetFileName) then
    begin
      Result.Success := True;
      Result.BytesProcessed := 0;
      Exit;
    ...
    try
      ...
      // 根据源编码进行转换
      if (SourceEncoding = ENCODING_UTF8) or (SourceEncoding = ENCODING_UTF8_BOM) then
      begin
        // 已经是UTF-8，只需添加BOM
      ...
      begin
        ...
        if SourceEncoding = ENCODING_ANSI then
          SourceCodePage := GetACP() // 使用系统默认ANSI代码页
        else if SourceEncoding = ENCODING_GBK then
          SourceCodePage := 936 // GBK代码页
        else if SourceEncoding = ENCODING_BIG5 then
...
begin
  ...
  try
    ...
    // 如果已经是UTF-8无BOM，并且源文件和目标文件相同，则不需要处理
    if (SourceEncoding = ENCODING_UTF8) and (SourceFileName = TargetFileName) then
    begin
      Result.Success := True;
      Result.BytesProcessed := 0;
      Exit;
    ...
    try
      ...
      if (SourceEncoding = ENCODING_UTF8) or (SourceEncoding = ENCODING_UTF8_BOM) then
      begin
        // 已经是UTF-8，只需移除BOM（如果有）
        ConvertedBuffer := Buffer; // BOM已经在读取时跳过
      end
      else
      begin
        // 需要从其他编码转换为UTF-8
...
begin
  ...
  Result := (BOMResult.BOMType = bomUTF8);
end;

class function TUTF8BOMConverter_Improved.IsUTF8Buffer(const Buffer: TBytes; out HasBOM: Boolean): Boolean;
var
  UTF8Result: TUTF8DetectionResult;
begin
